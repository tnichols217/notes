...
![[CSDS 302 - Discrete Math/HW/9/10.2/2b.svg|500]]
...
Let $G$ be the graph with vertices $v_1,v_2,v_3$ and with $\mathbf A$ as its adjacency matrix. Use the answers to part (a) to find the number of walks of length $2$ from $v_1$ to $v_3$ and the number of walks of length $3$ from $v_1$ to $v_3$. Do not draw $G$ to solve this problem.
