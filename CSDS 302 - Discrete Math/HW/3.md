...
> [!answer]
> False, There exists a product of any irrational number and any rational number that is irrational.
...
> [!answer]
> Contrapositive (True): $\exists n\in\mathbf Z$ if $n$ is even and $n\not=2$ then $n$ is not prime
> Converse: $\forall n\in\mathbf Z$ if $n$ is off or $n=2$ then $n$ is prime
