...
As you can see, the outputs of $p$ and $p\land\mathbf t$ are the same. This is because asking if ($p$ and true) is true, it only depends on whether $p$ itself is true
```
...
As you can see, the outputs of $p\land\mathbf c$ and $p\lor\mathbf c$ are not the same. This is because taking an "and" with something that is always false is not the same as taking an "or" with something that is always false. Taking an "and" with something that is always false will always be false, as seen in the table. Taking an "or" with something that is always false will just be equivalent to the first argument of the "or".
```
...
Yes, these are the same by the distributive law, and can be seen in the table as the two output colu
