...
> $n\vert m\to m< n$
> $m<n\to P(m)$
> Since $n$ can be composited into $m, \frac{n}{m}$, $n$ will have all the factors of $m$ and $\frac{n}{m}$ as multiplication is communicative, any satisfactory $m$ will ultimately give the same factors
> 
