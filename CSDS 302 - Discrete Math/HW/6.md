...
> [!answer]
> The units digit of $4^n:\forall n\in\mathbf Z^+$ will be equal to $5+(-1)^n\in\{4,6\}\iff10$
...
> 
> If $U_{n-1}=4$
> $n-1\mod 2=1$ as only odd powers of $-1$ would make $(-1)^n$ negative, making $U_{n-1}<5$
> $4^n=4^{n-1}4=10(4M)+4U_{n-1}$
> $4U_{n-1}=16$
> $4^n=10(4M+1)+6$
> $\therefore U_n=6$
> $n-1\mod 2=1\to m\mod 2=0$
# 5.6
