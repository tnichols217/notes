...
“Proof: Suppose $A$ and $B$ are any sets, and suppose $x\in(A-B)\cup(A\cap B)$. If $x\in A$ then $$
and so, by definition of difference, x [ A and
x Ó B. In particular, x [ A, and, therefore,
(A 2 B) ø (A ù B) # A by definition of subset.”
