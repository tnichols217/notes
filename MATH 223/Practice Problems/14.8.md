...
$=\begin{cases}x_j=e^{\lambda-1}e^{\rho E_j}\qquad\forall x_j\in\{x_1,...,x_n\}\\\sum\limits_{j=1}^ne^{\rho E_j}=N/e^{\lambda-1}\\\sum\limits_{j=1}^nE_je^{\rho E_j}=E/e^{\lambda-1}\end{cases}$
