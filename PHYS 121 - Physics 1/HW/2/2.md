...
Two blocks are placed together on a table as shown. Block A has a given mass $m_A$ and Block B has a given mass mB . There is friction acting on the surfaces between the two blocks. However, the surface of the table is frictionless. An instructor pushes horizontally on Block B with his finger with a constant given (known) applied force Fapp . Assume also that known values of both the coefficient of static friction μs and the coefficient of kinetic friction μk are given parameters. Finally, here we observe that the given applied force Fapp is small enough so that Block A neither slips nor slides relative to Block B. (In other words, Block A stays stuck to Block B).