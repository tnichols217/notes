...
~r = [P cos(Ωt) + Q cos(ωt)]ı̂ + [P sin(Ωt) + Q sin(ωt)]̂

Where $P = 4 m$, $Q = 1 m$, $\Omega = 2\pi$ radians per second, and ω = 30π radians per second.
The plot shown corresponds to the path of the particle for 0 < t < 1 second. Note that Point A as
shown corresponds to the position at both t = 0 and t = 1 second.
