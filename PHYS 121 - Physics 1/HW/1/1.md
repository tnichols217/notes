...
> 
> Since the block moves horizontally across the floor, and we are given $\theta$, which is the deviation from this horizontal movement of which the applied force is applied, $\theta+\pi$ represents the angle between movement and force, which is needed to calculate work through the dot product. As such, work is simply the product of distance traveled $D$, magnitude of force $F_{app}$, and the cosine of the angle $\cos(\theta)$
> 
> ---
> 
> $W_{app}=-F_{app}D\cdot\cos(\theta)$