...
> $W_{f}=F_{f}D\cdot\cos(\theta+\pi)$
> $W_{f}=-F_{f}D\cdot\cos(\theta)$
> 
> Since the block moves horizontally across the floor, and we are given $\theta$, which is the deviation from this horizontal movement of which the applied force is applied, $\theta+\pi$ represents the angle between movement and force as friction force is pointing $\pi$ radians the other way, which is needed to calculate work through the dot product. As such, work is simply the product of distance traveled $D$, magnitude of force $F_{f}$, and the cosine of the angle $\cos(\theta+\pi)$
> 
> ---
> 
> $W_{f}=-F_{f}D\cdot\cos(\theta)$
...
> $W_{net}=F_{app}D\cdot\cos(\theta)+-F_{f}D\cdot\cos(\theta)$
> $W_{net}=(F_{app}-F_{f})D\cdot\cos(\theta)$
> 
> ---
> 
> $W_{net}=(F_{app}-F_{f})D\cdot\cos(\theta)$
...
> $K_f=\frac{1}{2}Mv_f^2$
> $(F_{app}-F_{f})D\cdot\cos(\theta)=\frac{1}{2}Mv_f^2$
> $v_f=\sqrt\frac{2(F_{app}-F_{f})D\cdot\cos(\theta)}{M}$
> 
> ---
> 
> $v_f=\sqrt\frac{2(F_{app}-F_{f})D\cdot\cos(\theta)}{M}$
...
> [!answer]
> Since we know all other forces on the block have $0$ work, we can assume the net work on the block is all due to the work due to weight.
> 
> $\Delta E=W$
> $E_i=U_i+K_i$
> $E_i=m_bgh_i+\frac{1}{2}m_bV_i^2$
> $E_f=m_bgh_f+\frac{1}{2}m_bV_f^2$
