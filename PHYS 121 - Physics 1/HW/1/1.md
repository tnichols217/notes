...
> 
> Since the block moves horizontally across the floor, and we are given $\theta$, which is the deviation from this horizontal movement of which the applied force is applied, $\theta$ also represents the angle between movement and force, which is needed to calculate work through the dot product. As such, work is simply the product of distance traveled $D$, 
