...
> $y(t)=x_0+\int\limits_0^tv_{y0}+\int\limits_0^ta_y(s)~ds~ds\qquad$ Integrating the acceleration and velocity to calculate position — derives the free fall equations.
...
José Ramı́rez swings the bat, but hits a pop-up. The baseball has a vertical upward velocity of $v_0 = 23.7 \frac{m}{s}$ meters per second when the ball leaves his bat. Relative to his bat, how high in the air will the baseball travel? How much total time will elapse before the ball is caught by the opposing catcher? Assume the ball is caught at the same vertical location that it was hit. Ignore air resistance.