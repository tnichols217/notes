...
> $y(t)=x_0+\int\limits_0^tv_{y0}+\int\limits_0^ta_y(t)~dt~dt$ Integrating the acceleration and velocity to calculate position — derives the free fall equations.
...
> [!answer]
> As we established in the previous section (a), we know we hit the ground at $T=\sqrt{\frac{2H}{g}}$