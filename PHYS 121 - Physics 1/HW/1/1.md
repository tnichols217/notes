...
> $y(t)=x_0+\int\limits_0^tv_{y0}+\int\limits_0^ta_y(s)~ds~ds$ Integrating the acceleration and velocity to calculate position — derives the free fall equations.
> $y(t)=H+\frac{1}{2}a_yt^2$ We know we start at rest at height $H$
> $y(T)=H+\frac{1}{2}a_yT^2$ Plug in final time
> $-\frac{2H}{a_y}=T^2$ We know we end at $y(T)=0$
> $\frac{2H}{g}=T^2$ Substitute $a_y$
> $T=\sqrt{\frac{2H}{g}}$
> 
> ---
> 
...
> $v_y(T)=v_{y0}+\int\limits_{0}^{T}a_y~dt$ Integrate acceleration to get velocity
> $v_y(T)=Ta_y$
> $v_y(T)=\sqrt{\frac{2H}{g}}g$
> $v_y(T)=\sqrt{\frac{2Hg}g$
