...
A massive uniform rope of total mass m and length L is attached to two trees as shown in the
figure above. The connection point for the rope is the same height on each tree, each rope makes
an given angle θ with each tree as shown.