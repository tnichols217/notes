...
Two blocks A and B each with mass $m= 2.0~kg$ sit on a frictionless surface. Block A has a velocity $v0 = 5.0 m/s$ towards block B which is at rest. There is a massless spring with spring
constant k =1.00 N/m attached to Block B as shown. Block A collides with the spring that is
attached Block B. The spring is compressed, and then after a while it is uncompressed and the two
blocks separate from each other. Note that because the spring force is Conservative, the collision
is totally elastic.