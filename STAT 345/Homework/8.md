...
$=\begin{cases}0&x<0\cup y<0\\(bx^2/2+b^2x)/4\bigg|_0^y&0\le x< 2\cap0\le y<1\\\int\limits_0^y(2b+2b^2)/4~db&2\le x\cap 0\le y<1\\\int\limits_0^1(bx^2/2+b^2x)/4~db&0\le x<2\cap 1\le y\\1&2\le x\cap1\le y\end{cases}$
