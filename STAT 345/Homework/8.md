...
$=\begin{cases}0&x<0\cup y<0\\\int\limits_0^y\int\limits_0^x(a+2b)/4~da~db&0\le x< 2\cap0\le y<1\\&2\le x\cap 0\le y<1\\&0\le x<2\cap 1\le y\\&2\le x\cap1\le y\end{cases}$
