...
$=\begin{cases}0&x<0\cup y<0\\\int\limits_0^y(x^2/2+2bx)/4~db&0\le x< 2\cap0\le y<1\\\int\limits_0^y(2+4b)/4~db&2\le x\cap 0\le y<1\\\int\limits_0^1(a^2/2+2ba)/4\bigg|_0^x~db&0\le x<2\cap 1\le y\\1&2\le x\cap1\le y\end{cases}$
