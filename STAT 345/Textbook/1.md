...
Let $S=\{s_1,...,s_n\}$ be a finite set. Let $B$ be any $\sigma$ algebra of $S$. Let $p_1,...,p_n$ be non-negative numbers that sum to 1. For any $A\in B$, define $P(A)$ as $\sum\limits_{\{\}}$